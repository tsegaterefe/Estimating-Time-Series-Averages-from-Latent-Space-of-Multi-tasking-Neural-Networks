...
class One_NN:
    ...
    def classify_DTW(self):
        ...
        for i in range(self.mytestseries.shape[0]):
            for j in range(self.class_distance.shape[1]):
                ...
                #align.findwarppath()
                self.class_distance[0,j]=metrics.dtw(self.mytestseries[i,:],self.myclassavgs[j,:])#math.sqrt(sum(math.square(align.Warpedfv-align.Warpedsv))) #tsl.metrics.dtw(self.mytestseries[i,:],self.myclassavgs[j,:])#metrics.dtw(self.mytestseries[i,:],self.myclassavgs[j,:]) #metrics.dtw(self.mytestseries[i,:],self.myclassavgs[j,:])#math.sqrt(sum(math.square(align.Warpedfv-align.Warpedsv))) #tsl.metrics.dtw(self.mytestseries[i,:],self.myclassavgs[j,:]) 
    ...
    def plot_classifier_result(self,data,label):
        correct_count=0
        for i in range(data.shape[0]):
            maxs=npy.where(data[i,:]==max(data[i,:]))
            self.confusion_matrix[int(label[i,0]),maxs[0][0]]+=1
            if maxs[0]==label[i]:
                correct_count+=1
        return correct_count/data.shape[0]*100 
    def classify_Euclidean(self):
        correct_count=0
        print('In Euclidean classif class avg',self.myclassavgs.shape)
        for i in range(self.mytestseries.shape[0]):
            for j in range(self.class_distance.shape[1]):
